...
public class ParamInterceptor implements Interceptor {
    ...
    public interface OnInterceptListener {
        boolean onIntercept(ParamInterceptor interceptor);
    }
    ...
    public void registerInterceptListener(OnInterceptListener listener) {
        mListener = listener;
    }

    public void unregisterInterceptListener() {
        mListener = null;
    }
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (mListener != null) {
            shouldIntercept = mListener.onIntercept(this);
        }
        if (!shouldIntercept || Ifs.isNullOrEmpty(mParams)) {
    ...
    public static String mergeParamsForUrl(String url, Map<String, String> params) {
        if (TextUtils.isEmpty(url) || Ifs.isNullOrEmpty(params)) return url;
    ...
    {
        if (builder == null || Ifs.isNullOrEmpty(params)) return builder;
        Iterator<Map.Entry<String, String>> paramEntries = params.entrySet().iterator();
        while (paramEntries.hasNext()) {
            Map.Entry<String, String> paramEntry = paramEntries.next();
    ...
    public static RequestBody mergeParamsForBody(RequestBody original, Map<String, String> params) {
        if (!(original instanceof FormBody) || Ifs.isNullOrEmpty(params)) return original;
